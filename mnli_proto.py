...
class mnli_datamodule(pl.LightningDataModule):
    loader_columns = [
        "datasets_idx",
        "input_ids",
        "token_type_ids",
    ...
    def setup(self, stage: str):
        self.dataset = datasets.load_dataset('glue', 'mnli')
...
parser.add_argument('-l0', default=1, type=float, help='lambda 0 in loss')
parser.add_argument('-l1', default=.8, type=float, help='lambda 1 in loss')
parser.add_argument('-l2', default=.1, type=float, help='lambda 2 in loss')
parser.add_argument('-lr', default=3e-4, type=float, help='initial learning rate')
parser.add_argument('-optim_eps', default=1e-8, type=float, help='adam epsilon')
...
if __name__ == '__main__':
    ...
    mnli_dm = mnli_datamodule(model_name_or_path=args.model_name,
                        max_seq_length=args.max_seq_length,
                        train_batch_size=args.batch_size,
                        eval_batch_size=args.batch_size)

    mnli_dm.setup(stage='fit')
    ...
    tb_logger = TensorBoardLogger(f'{args.logger_dir}', name=f'mnli_tensorboard_logs')
    ...
    trainer.fit(proto, datamodule=mnli_dm)