...
class biossesDM(pl.LightningDataModule):
    loader_columns = [
        "datasets_idx",
        "input_ids",
        "token_type_ids",
    ...
    def __init__(
        ...
        self.text_fields = ['question', 'context']
        self.num_labels = 3 #yes, no, maybe
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_name_or_path, use_fast=True)

    def setup(self, stage: str):
        self.dataset = datasets.load_dataset('EMBO/BLURB', 'PubMedQA')
        ...
        for split in self.dataset.keys():
            ...
            self.dataset[split] = self.dataset[split].map(
                ...
                batched=True,
                remove_columns=['pubid', 'long_answer', 'final_decision'],
            )
    ...
    def val_dataloader(self):
        return DataLoader(self.dataset["train"], batch_size=self.eval_batch_size)
    ...
    def convert_to_features(self, example_batch, indices=None):
        input_texts = []
        for quest, con in zip(example_batch[self.text_fields[0]], example_batch[self.text_fields[1]]):
            input_texts.append(f'question: {quest} context: {con}')

        labels = [] #map decisions to fine labels
        for dec in example_batch["final_decision"]:
            if dec == 'no':
                labels.append(0)
            elif dec == 'maybe':
                labels.append(1)
        ...
        # Rename score to labels to make it easier to pass to model forward
...
parser.add_argument('-lambda0', default=1, type=float, help='lambda0 in loss')
parser.add_argument('-lambda1', default=0.5, type=float, help='lambda1 in loss')
parser.add_argument('-lambda2', default=0.5, type=float, help='lambda2 in loss')
...
if __name__ == '__main__':
    ...
    # get data module
    pubmedqa_dm = biossesDM(model_name_or_path=args.model_name,
                        max_seq_length=args.max_seq_length,
                        train_batch_size=args.batch_size,
                        eval_batch_size=args.batch_size)


    pubmedqa_dm.setup(stage='fit')

    # if loading a trained model
    if args.load_model != '':
        print(f'loading a model: {args.load_model}')
        proto = proto_lm.load_from_checkpoint(args.load_model,
                                                   pretrained_model=AutoModelForSequenceClassification.from_pretrained(args.model_name, ignore_mismatched_sizes=True).roberta)
    else:
        # otherwise build proto object

        # config = AutoConfig.from_pretrained(args.model_name)
        llm_model = AutoModelForSequenceClassification.from_pretrained(args.model_name,
                                                                       ignore_mismatched_sizes=True) #roberta based model
    ...
    tb_logger = TensorBoardLogger(f'{args.logger_dir}', name=f'{args.config_subdir}')
    ...
    if args.load_model == '':  # no model to load
        trainer.fit(proto, datamodule=pubmedqa_dm)
    else:
        print(f'loading model: {args.load_model}')
        trainer.fit(proto, datamodule=pubmedqa_dm, ckpt_path=args.load_model)

    trainer.test(proto, datamodule=pubmedqa_dm)