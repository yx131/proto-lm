...
class biossesDM(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.num_labels = 1 #becuase regression
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_name_or_path, use_fast=True)

    def setup(self, stage: str):
        self.dataset = datasets.load_dataset('qanastek/Biosses-BLUE')
        # self.dataset = datasets.load_dataset('EMBO/BLURB', 'BIOSSES')
        ...
        for split in self.dataset.keys():
            ...
            self.dataset[split] = self.dataset[split].map(
                ...
                batched=True,
                remove_columns=['score'], #don't need score here, add it as 'labels'
            )
    ...
    def convert_to_features(self, example_batch, indices=None):
        ...
        features["labels"] = example_batch["score"]
...
if __name__ == '__main__':
    ...
    # get data module
    bioss_dm = biossesDM(model_name_or_path=args.model_name,
                        max_seq_length=args.max_seq_length,
                        train_batch_size=args.batch_size,
                        eval_batch_size=args.batch_size)


    bioss_dm.setup(stage='fit')
    ...
    if args.load_model == '':  # no model to load
        trainer.fit(proto, datamodule=bioss_dm)
    else:
        print(f'loading model: {args.load_model}')
        trainer.fit(proto, datamodule=bioss_dm, ckpt_path=args.load_model)

    trainer.test(proto, datamodule=bioss_dm)